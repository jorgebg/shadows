...
<div>
    <TabBar tabs={screenIDs} let:tab={screenID} bind:active={activeScreenID}>
        ...
        <Tab
            ...
            data-keybind={screen.title[0].toLowerCase()}
        ...
        >
            {#if typeof screen.icon === "object"}
                ...
                >
            {:else}
                <TabIcon class="material-symbols-outlined"
                    >{screen.title[0]}</TabIcon
                >
            {/if}
            <TabLabel class="screen-tab-label">{screen.title}</TabLabel>
        </Tab>
    </TabBar>
                    ...
                    <Cell {spanDevices}>
                        <div class="screen-panel screen-options">
                            <div class="screen-options-header">
                                <div>
                                    <div
                                        class="mdc-typography--overline option-selection-path"
                                    >
                                        {$activeOption.breadcrumbs
                                            .map((o) => o.title)
                                            .join(" / ")}
                                    </div>
